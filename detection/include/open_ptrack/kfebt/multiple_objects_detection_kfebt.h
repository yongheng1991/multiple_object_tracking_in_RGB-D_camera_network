...
#include "open_ptrack/kfebt/kfebtracker.h"
//#include "trax.h"
...
private:
    ...
    ros::Publisher detection_pub,asms_detection_pub;
    ...
    //asms
    std::vector<std::shared_ptr<KFebTracker> > color_trackers;
    //    KFebTracker test_tracker;
    //    KFebTracker tracker;
...
public:
          ...
          nh(), spinner(0), it(nh) ,show_2D_tracks(show_2D_tracks)
    {
        ...
        asms_detection_pub=nh.advertise<DetectionArray>("/objects_detector/detections",3);
    }
    ...
    void run_detection(){
        ...
        //        for(size_t i=0; i<color_trackers.size(); i++)
        //        {
        //            std::shared_ptr<KFebTracker> tmp_tracker=color_trackers[i];
        //        }
...
private:
    ...
    void select_rois_from_gui()
    {
        ...
        for (std::vector<pair<string,Rect>>::const_iterator roi_it= rois_from_gui.begin();roi_it!=rois_from_gui.end();roi_it++)
        {
            ...
            //            KFebTracker tracker;
            //            tracker.init("AKN");
            //            tracker.initTrackers(main_color, (*roi_it).second);
            //            color_trackers.push_back(tracker);
    ...
    {
        std::string init_filename="/home/zhao/recorded_rosbag/init_"+color_header_frameId.substr(1,color_header_frameId.size()-19)+".txt";
        //        std::cout<<"init_file name :  "<<init_filename<<std::endl;
        std::ifstream infile(init_filename);        int a,b,c,d;
        string object_name;
        ...
        while(infile >>object_name >> a  >> b  >> c  >> d)
        {
            ...
            std::shared_ptr<KFebTracker> tracker = std::make_shared<KFebTracker>();
            tracker->init("AKN");
            tracker->initTrackers(main_color, object_rect);
            color_trackers.push_back(tracker);
 }
    ...
    void multiple_objects_detection_main()    //main_detection
    {
        ...
        //        ground_truth_array_msg->header.frame_id = color_header_frameId;
        //        ground_truth_array_msg->header.stamp = ros::Time::now();
        ...
        for(size_t i=0; i<Object_Detectors.size(); i++)
        {
            ...
            if(occludes[i]==false)// if no occlusion
            {
                ...
                /// \brief test_Rect
                std::shared_ptr<KFebTracker> tmp_tracker = color_trackers[i];
                Rect test_Rect = tmp_tracker->track(main_color);


                //                Rect test_Rect = color_trackers[i].track(main_color);
                //                Rect test_Rect = test_tracker.track(main_color);
...
//                    //                std::cout<<"GT_x: "<<GT_transform.getOrigin().x()<<"  GT_y: "<<GT_transform.getOrigin().y()<<"  GT_z: "<<GT_transform.getOrigin().z()<<std::endl;
//                }
...
//                    detection_marker_msg->markers.push_back(ground_truth_msg2); //push the detecion marker for analysis the single camera detection in matlab
        ...
        asms_detection_pub.publish(detection_array_msg);		 // publish message
