#include "colotracker_.h"
#include <iostream>
#include <cmath>
#include <fstream>

void ColorTrackert::init(cv::Mat & img, int x1, int y1, int x2, int y2)
{
...
cv::Point ColorTrackert::histMeanShift(double x1, double y1, double x2, double y2){
    int maxIter = 15;
...
cv::Point ColorTrackert::histMeanShiftIsotropicScale(double x1, double y1, double x2, double y2, double * scale, int * iter, double* similarity)
{
    int maxIter = 15;
...
BBoxt * ColorTrackert::track(cv::Mat & img, double x1, double y1, double x2, double y2, double* confidence)
{
    double width = x2-x1;
    double height = y2-y1;
...
void ColorTrackert::preprocessImage(cv::Mat &img)
{
    cv::Mat ra[3] = {im1, im2, im3};
    cv::split(img, ra);
}

void ColorTrackert::extractBackgroundHistogram(int x1, int y1, int x2, int y2, Histogramt & hist)
{
    int offsetX = (x2-x1)/2;
    int offsetY = (y2-y1)/2;
...
void ColorTrackert::extractForegroundHistogram(int x1, int y1, int x2, int y2, Histogramt & hist)
{
    hist.clear();
