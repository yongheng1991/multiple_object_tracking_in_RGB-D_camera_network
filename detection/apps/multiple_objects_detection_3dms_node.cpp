...
#include "open_ptrack/3dms/multiple_objects_detection.h"
...
void configCb(detection::multiple_objects_detectionConfig &config, uint32_t level)
{
    ...
    Object_Detector::SMax=config.SMax;
    Object_Detector::VMax=config.VMax;

    Object_Detector::DMin=config.DMin;
    Object_Detector::DMax=config.DMax;

    Object_Detector::h_bins=config.h_bins;
    Object_Detector::s_bins=config.s_bins;
    Object_Detector::v_bins=config.v_bins;
...
int main(int argc, char** argv){
    ...
    nh.param("show_2D_track",show_2D_tracks,true);
    ...
    nh.param("init_rois_txt_path",init_rois_txt_path,std::string(" "));// for reading initial ROI from txt file
    ...
    nh.param("SMin",Object_Detector::SMin,0);
    nh.param("VMin",Object_Detector::VMin,0);
    nh.param("HMax",Object_Detector::HMax,180);
    nh.param("SMax",Object_Detector::SMax,256);
    nh.param("VMax",Object_Detector::VMax,256);

    nh.param("DMin",Object_Detector::DMin,0);
    nh.param("DMax",Object_Detector::DMax,255);

    nh.param("h_bins",Object_Detector::h_bins,18);
    nh.param("s_bins",Object_Detector::s_bins,16);
    nh.param("v_bins",Object_Detector::v_bins,8);

    nh.param("d_bins",Object_Detector::d_bins,255);

    nh.param("AREA_TOLERANCE",Object_Detector::AREA_TOLERANCE,20);
    nh.param("QUALITY_TOLERANCE",Object_Detector::QUALITY_TOLERANCE,100);
    nh.param("DENSITY_TOLORENCE",Object_Detector::DENSITY_TOLORENCE,0.2);

    nh.param("Backprojection_Mode",Object_Detector::Backprojection_Mode,std::string("HSVD"));

    nh.param("with_occ_handle",Object_Detector::with_occ_handle,true);
    ...
    Multiple_Objects_Detection _multiple_objects_detection(output_detection_topic,set_object_names,useExact, useCompressed,
                                                           use_background_removal,background_calculate_frames,threshold_4_detecting_foreground,show_2D_tracks,init_rois_txt_path);
